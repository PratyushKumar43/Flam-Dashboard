  ...
  return (
    <div className="relative min-h-screen w-full overflow-x-hidden bg-black text-white">
      ...
      <main className="relative z-10 mx-auto flex w-full max-w-6xl flex-col gap-24 px-6 pb-24 md:px-12 lg:px-16">
        <section
          id="get-started"
          className="grid gap-10 rounded-3xl bg-white/5 p-10 backdrop-blur-xl ring-1 ring-white/10 lg:grid-cols-[1.1fr_0.9fr]"
        >
          <div className="flex flex-col gap-6">
            <span className="text-sm font-light uppercase tracking-[0.2em] text-white/60">
              Overview
            </span>
            ...
            <p className="text-base font-light leading-relaxed text-white/70">
              This landing page translates the requirements from the assignment brief into actionable sections. Start by scaffolding the shadcn-inspired structure under <code className="rounded bg-white/10 px-2 py-0.5">/components/ui</code>, wire Tailwind v4 for styling, and keep TypeScript strict to ensure production-ready safety nets.
            </p>
            <div className="grid gap-6 sm:grid-cols-2">
              {featureHighlights.map((feature) => (
                <article
                  key={feature.title}
                  className="group flex flex-col gap-3 rounded-2xl border border-white/10 bg-black/50 p-5 transition-transform duration-300 hover:-translate-y-1 hover:border-white/20"
                >
                  <feature.icon className="h-6 w-6 text-cyan-300" />
                  <h3 className="text-lg font-light tracking-tight text-white">
                    {feature.title}
                  </h3>
                  <p className="text-sm font-extralight leading-relaxed text-white/70">
                    {feature.description}
                  </p>
                </article>
              ))}
          ...
          <div className="relative flex items-center justify-center">
            <div className="relative aspect-[4/5] w-full overflow-hidden rounded-3xl border border-white/10">
              ...
              <div className="absolute bottom-4 left-4 right-4 flex items-center justify-between text-xs font-light uppercase tracking-[0.2em] text-white/60">
        ...
        <section className="grid gap-8 lg:grid-cols-[0.9fr_1.1fr]">
          <div className="flex flex-col gap-4 rounded-3xl border border-white/10 bg-white/5 p-8 backdrop-blur-xl">
            <span className="text-sm font-light uppercase tracking-[0.2em] text-white/60">
              Benchmarks
            </span>
            ...
            <p className="text-sm font-extralight leading-relaxed text-white/70">
              Use <code className="rounded bg-white/10 px-2 py-0.5">usePerformanceMonitor</code> to track FPS, memory, and React commit cost. Keep the dashboard stable for hours, then log your results in <code className="rounded bg-white/10 px-2 py-0.5">PERFORMANCE.md</code>.
            </p>
            <div className="grid gap-4 sm:grid-cols-3">
              {benchmarkMetrics.map((metric) => (
                <div
                  key={metric.title}
                  className="flex flex-col gap-2 rounded-2xl border border-white/10 bg-black/60 p-5 text-left"
                >
                  <span className="text-3xl font-light text-cyan-300">{metric.title}</span>
                  <p className="text-xs font-extralight leading-relaxed text-white/60">
                    {metric.caption}
                  </p>
                </div>
              ))}
          ...
          <div className="relative overflow-hidden rounded-3xl border border-white/10">
            ...
            <div className="absolute bottom-0 left-0 right-0 space-y-3 bg-gradient-to-t from-black/80 via-black/60 to-transparent p-8">
        ...
        <section
          id="showcase"
          className="flex flex-col gap-10 rounded-3xl bg-white/5 p-10 backdrop-blur-xl ring-1 ring-white/10"
        >
          ...
          <div className="grid gap-6 lg:grid-cols-4">
            {pipelineMilestones.map((milestone) => (
              <div
                key={milestone.label}
                className="flex flex-col gap-3 rounded-2xl border border-white/10 bg-black/60 p-5"
              >
                <milestone.icon className="h-6 w-6 text-purple-300" />
                <h3 className="text-lg font-light tracking-tight text-white">
                  {milestone.label}
                </h3>
                <p className="text-xs font-extralight leading-relaxed text-white/70">
                  {milestone.detail}
                </p>
              </div>
            ))}
        ...
        <section className="grid gap-8 rounded-3xl border border-white/10 bg-black/70 p-10">
          <div className="space-y-4">
            ...
            <p className="text-sm font-extralight leading-relaxed text-white/70">
              Document your setup in <code className="rounded bg-white/10 px-2 py-0.5">README.md</code>, capture benchmarks in <code className="rounded bg-white/10 px-2 py-0.5">PERFORMANCE.md</code>, and deploy to Vercel for a live, stress-tested demo.
            </p>
          </div>
          <div className="grid gap-6 lg:grid-cols-3">
            <div className="flex flex-col gap-3 rounded-2xl border border-white/10 bg-white/10 p-5 text-black">
            ...
            <div className="flex flex-col gap-3 rounded-2xl border border-white/15 bg-white/5 p-5">
        ...
        <section className="overflow-hidden rounded-3xl border border-white/10">
          <div className="relative h-full w-full">
            ...
            <div className="absolute bottom-0 left-0 right-0 p-10 text-white">
              ...
              <p className="mt-4 max-w-3xl text-sm font-extralight leading-relaxed text-white/75">
