...
import { clearCanvas, drawLineChart, drawGrid, drawAxes, pointToCanvas } from '@/lib/canvasUtils'
...
export const LineChart = memo(function LineChart({
  ...
  const chartConfig: ChartConfig = useMemo(() => {
    const valueRange = stats.max - stats.min || 1 // Prevent division by zero
    return {
  ...
  const render = useCallback(() => {
    ...
    // Draw line chart
    if (data.length > 0) {
      drawLineChart(ctx, data, chartConfig, viewport, color, 2)
    }
